...
const {axios,getToken,user,image_base_url} = useAppContext()
  ...
  const getMyBooking = async () => {
   try{
 const { data } = await axios.post('/api/user/bookings',{headers: {Authorizaton: `Bearer ${await getToken()}`}})
 if(data.success){
  ...
  return !isLoading ? (
    <div className='relative px-6 md:px-16 lg:px-40 pt-30 md:pt-40 min-h-[80vh]'>
      ...
      {bookings.map((item, index) => (
        <div key={item.id} className='flex flex-col md:flex-row justify-between bg-primary/8 border border-primary/20 rounded-lg mt-4 p-2 max-w-3xl'>
